...
#if 0
        SGX_DBG(DBG_E, "\tFreeing EDMM [%p, %p)\n",
                free_addr, free_addr + free_size);
        if (free_addr != addr || free_size != size) {
            SGX_DBG(DBG_E, "%s:%d freeing [%p, %p), vma [%p, %p), free [%p, %p)\n", __func__, __LINE__,
                    addr, addr + size,
...
int zero_enclave_pages(void *start, size_t size)
{
    ...
    if (pal_sec.edmm_mode == SGX_EDMM_MODE_NOEDMM) {
        memset(start, 0, size);
        return 0;
    }
