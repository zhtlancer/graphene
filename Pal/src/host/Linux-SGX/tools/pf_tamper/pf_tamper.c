...
static void derive_main_key(const pf_key_t* kdk, const pf_keyid_t* key_id, pf_key_t* out_key) {
    ...
    buf.node_number = 0;
    memcpy(&buf.nonce32, key_id, sizeof(buf.nonce32));
    buf.output_len = 0x80;
...
void make_output_path(const char* suffix) {
...
- Node 0: metadata (meta_data_node_t)
  - meta_data_plain_t
  - meta_data_encrypted_t (may include MD_USER_DATA_SIZE bytes of data)
  - meta_data_padding_t
...
- ...
*/
void truncate_file(const char* suffix, size_t output_size) {
    int ret;
...
void tamper_truncate(void) {
    size_t mdps = sizeof(meta_data_plain_t);
    DBG("size(meta_data_plain_t)             = 0x%04lx\n", sizeof(meta_data_plain_t));
    DBG("meta_data_plain_t.file_id           : 0x%04lx (0x%04lx)\n",
        offsetof(meta_data_plain_t, file_id), FIELD_SIZEOF(meta_data_plain_t, file_id));
    DBG("meta_data_plain_t.major_version     : 0x%04lx (0x%04lx)\n",
        offsetof(meta_data_plain_t, major_version), FIELD_SIZEOF(meta_data_plain_t, major_version));
    DBG("meta_data_plain_t.minor_version     : 0x%04lx (0x%04lx)\n",
        offsetof(meta_data_plain_t, minor_version), FIELD_SIZEOF(meta_data_plain_t, minor_version));
    DBG("meta_data_plain_t.meta_data_key_id  : 0x%04lx (0x%04lx)\n",
        offsetof(meta_data_plain_t, meta_data_key_id),
        FIELD_SIZEOF(meta_data_plain_t, meta_data_key_id));
    DBG("meta_data_plain_t.meta_data_gmac    : 0x%04lx (0x%04lx)\n",
        offsetof(meta_data_plain_t, meta_data_gmac),
        FIELD_SIZEOF(meta_data_plain_t, meta_data_gmac));
    DBG("meta_data_plain_t.update_flag       : 0x%04lx (0x%04lx)\n",
        offsetof(meta_data_plain_t, update_flag), FIELD_SIZEOF(meta_data_plain_t, update_flag));

    DBG("size(meta_data_encrypted_t)         = 0x%04lx\n", sizeof(meta_data_encrypted_t));
    DBG("meta_data_encrypted_t.clean_filename: 0x%04lx (0x%04lx)\n",
        mdps + offsetof(meta_data_encrypted_t, clean_filename),
        FIELD_SIZEOF(meta_data_encrypted_t, clean_filename));
    DBG("meta_data_encrypted_t.size          : 0x%04lx (0x%04lx)\n",
        mdps + offsetof(meta_data_encrypted_t, size), FIELD_SIZEOF(meta_data_encrypted_t, size));
    DBG("meta_data_encrypted_t.mht_key       : 0x%04lx (0x%04lx)\n",
        mdps + offsetof(meta_data_encrypted_t, mht_key),
        FIELD_SIZEOF(meta_data_encrypted_t, mht_key));
    DBG("meta_data_encrypted_t.mht_gmac      : 0x%04lx (0x%04lx)\n",
        mdps + offsetof(meta_data_encrypted_t, mht_gmac),
        FIELD_SIZEOF(meta_data_encrypted_t, mht_gmac));
    DBG("meta_data_encrypted_t.data          : 0x%04lx (0x%04lx)\n",
        mdps + offsetof(meta_data_encrypted_t, data), FIELD_SIZEOF(meta_data_encrypted_t, data));

    DBG("size(meta_data_padding_t)           = 0x%04lx\n", sizeof(meta_data_padding_t));
    DBG("meta_data_padding_t                 : 0x%04lx (0x%04lx)\n",
        mdps + sizeof(meta_data_encrypted_t), sizeof(meta_data_padding_t));
/*
typedef struct {
    uint64_t   file_id;
...
*/
    ...
    truncate_file("trunc_meta_plain_1", FIELD_TRUNCATED(meta_data_plain_t, file_id));
    truncate_file("trunc_meta_plain_2", offsetof(meta_data_plain_t, major_version));
    truncate_file("trunc_meta_plain_3", offsetof(meta_data_plain_t, minor_version));
    truncate_file("trunc_meta_plain_4", offsetof(meta_data_plain_t, meta_data_key_id));
    truncate_file("trunc_meta_plain_5", FIELD_TRUNCATED(meta_data_plain_t, meta_data_key_id));
    truncate_file("trunc_meta_plain_6", offsetof(meta_data_plain_t, meta_data_gmac));
    truncate_file("trunc_meta_plain_7", FIELD_TRUNCATED(meta_data_plain_t, meta_data_gmac));
    truncate_file("trunc_meta_plain_8", offsetof(meta_data_plain_t, update_flag));

    /* encrypted metadata */
    truncate_file("trunc_meta_enc_0", mdps + offsetof(meta_data_encrypted_t, clean_filename));
    truncate_file("trunc_meta_enc_1", mdps + FIELD_TRUNCATED(meta_data_encrypted_t, clean_filename));
    truncate_file("trunc_meta_enc_2", mdps + offsetof(meta_data_encrypted_t, size));
    truncate_file("trunc_meta_enc_3", mdps + FIELD_TRUNCATED(meta_data_encrypted_t, size));
    truncate_file("trunc_meta_enc_4", mdps + offsetof(meta_data_encrypted_t, mht_key));
    truncate_file("trunc_meta_enc_5", mdps + FIELD_TRUNCATED(meta_data_encrypted_t, mht_key));
    truncate_file("trunc_meta_enc_6", mdps + offsetof(meta_data_encrypted_t, mht_gmac));
    truncate_file("trunc_meta_enc_7", mdps + FIELD_TRUNCATED(meta_data_encrypted_t, mht_gmac));
    truncate_file("trunc_meta_enc_8", mdps + offsetof(meta_data_encrypted_t, data));
    truncate_file("trunc_meta_enc_9", mdps + FIELD_TRUNCATED(meta_data_encrypted_t, data));

    /* padding */
    truncate_file("trunc_meta_pad_0", mdps + sizeof(meta_data_encrypted_t));
    truncate_file("trunc_meta_pad_1", mdps + sizeof(meta_data_encrypted_t)
                  + sizeof(meta_data_padding_t) / 2);
/*
typedef struct {
...
void* create_output(const char* path) {
    void* mem = MAP_FAILED;
...
void pf_decrypt(const void* encrypted, size_t size, const pf_key_t* key, const pf_mac_t* mac,
...
void pf_encrypt(const void* decrypted, size_t size, const pf_key_t* key, pf_mac_t* mac,
...
#define __BREAK_PF(suffix, ...) do { \
    ...
    pf_decrypt(&meta->encrypted_part, sizeof(meta->encrypted_part), &g_meta_key, \
               &meta->plain_part.meta_data_gmac, meta_dec, "metadata"); \
    ...
    if (update) { \
        __BREAK_PF(suffix "_fixed", __VA_ARGS__ { \
                       pf_encrypt(meta_dec, sizeof(*meta_dec), &g_meta_key, \
                                  &meta->plain_part.meta_data_gmac, meta->encrypted_part, \
                                  "metadata"); \
                   } ); \
    } \
} while (0)
...
void tamper_modify(void) {
    meta_data_node_t* meta = NULL;
    uint8_t* out = NULL;
    meta_data_encrypted_t* meta_dec = malloc(sizeof(*meta_dec));
    if (!meta_dec)
        FATAL("Out of memory\n");
...
*/
    ...
    /* meta_data_key_id is the keying material for encrypted metadata key derivation,
     * so create also PFs with updated MACs
     */
    BREAK_PF("meta_plain_keyid_0", /*update=*/true,
             { meta->plain_part.meta_data_key_id[0] ^= 1; });
    BREAK_PF("meta_plain_keyid_1", /*update=*/true,
             { LAST_BYTE(meta->plain_part.meta_data_key_id) ^= 0xfe; });
    BREAK_PF("meta_plain_mac_0", /*update=*/true,
             { meta->plain_part.meta_data_gmac[0] ^= 0xfe; });
    BREAK_PF("meta_plain_mac_1", /*update=*/true,
             { LAST_BYTE(meta->plain_part.meta_data_gmac) &= 1; });

    BREAK_PF("meta_plain_update_flag_0", /*update=*/false,
             { meta->plain_part.update_flag = 0xff; });
/*
typedef struct {
...
*/
    BREAK_PF("meta_enc_filename_0", /*update=*/true,
             { meta_dec->clean_filename[0] = 0; });
    BREAK_PF("meta_enc_filename_1", /*update=*/true,
             { meta_dec->clean_filename[0] ^= 1; });
    BREAK_PF("meta_enc_filename_2", /*update=*/true,
             { LAST_BYTE(meta_dec->clean_filename) ^= 0xfe; });
...
int main(int argc, char *argv[]) {
    ...
    derive_main_key(&g_wrap_key, &((meta_data_plain_t*)g_input_data)->meta_data_key_id,
                    &g_meta_key);
